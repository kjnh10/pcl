...
rep(k, n) {
    rep(i, n) {
        rep(j, n) { chmin(d[i][j], d[i][k] + d[k][j]); }
    }
}
dump_2d(d, n, n);
...
rep(i, 1, n + 1) {  //桁数
    rep(leq, 2) {   //未満確定フラグ
        ...
        rep(d, lim + 1) { dp[leq || d < lim][i] += dp[leq][i - 1]; }
    }
}
...
void pairsort(vector<T>& x, vector<T>& y) {
    ...
    rep(i, n) { t[i] = make_tuple(x[i], y[i]); }
    sort(all(t));
    rep(i, n) {
        x[i] = get<0>(t[i]);
        y[i] = get<1>(t[i]);
...
template <class T>
vector<vector<vector<T>>> make_vector_2d(int n1, int n2, T init_value) {
    return vector<vector<T>>(n1, vector<T>(n2, init_value));
}

template <class T>
vector<vector<vector<T>>> make_vector_3d(int n1, int n2, int n3, T init_value) {
    return vector<vector<vector<T>>>(
        n1, vector<vector<T>>(n2, vector<T>(n3, init_value)));
}
...
#line 172 "library/cpp/misc/misc_snip.cpp"
